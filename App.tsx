      ...
      `}>
        ...
        <div className="p-6 md:p-12 flex-1 flex flex-col overflow-y-auto">
              ...
              return (
                <div key={step.id} className="relative group">
                  ...
                  {index !== STEPS.length - 1 && (
                    <div className="hidden md:block absolute left-[19px] top-10 bottom-0 w-[1px] bg-corporate-800 h-8 md:h-12 z-0"></div>
                  )}
                  
                  <div className={`
                    relative z-10 flex items-start gap-4 p-3 rounded-md transition-all duration-300
                    ${isActive ? 'bg-corporate-800 md:translate-x-2 shadow-sm' : 'hover:bg-corporate-800/50'}
                  `}>
                    <div className={`
                      w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 border-2 transition-all duration-300
                      ${isActive ? 'bg-white border-white text-corporate-900 scale-105 md:scale-110 shadow-lg' : 
                        isCompleted ? 'bg-corporate-900 border-green-500 text-green-500' : 
                        'bg-corporate-900 border-corporate-700 text-corporate-500'}
                    `}>
                      {isCompleted ? <CheckCircle2 size={20} /> : <span className="font-serif font-bold">{step.id}</span>}
                    </div>
                    <div className="pt-2">
                      <p className={`text-xs font-bold uppercase tracking-wider mb-0.5 transition-colors ${isActive ? 'text-white' : 'text-corporate-400'}`}>
                        {step.subtitle}
                      </p>
                      <h3 className={`font-serif font-medium text-lg leading-none transition-colors ${isActive ? 'text-white' : 'text-corporate-300'}`}>
                        {step.title}
                      </h3>
                    </div>
                  </div>
          ...
          <div className="mt-auto pt-8 md:pt-12">
            <div className="p-4 md:p-6 bg-corporate-800 rounded-sm border border-corporate-700">
              <h4 className="font-serif text-white text-base md:text-lg mb-2">Need Assistance?</h4>
              <p className="text-corporate-300 text-xs md:text-sm leading-relaxed">
      ...
      <div className="flex-1 flex flex-col min-w-0 bg-slate-50 overflow-y-auto h-screen w-full">
        <main className="flex-1 p-4 md:p-12 lg:p-16 w-full max-w-4xl mx-auto">
          ...
          <div className="bg-white p-5 md:p-12 shadow-paper rounded-sm border border-slate-100 min-h-[400px] md:min-h-[500px] overflow-hidden">
            <div 
              key={currentStep} 
              className={`space-y-6 md:space-y-8 ${direction === 'next' ? 'animate-slideInRight' : 'animate-slideInLeft'}`}
            >
              {currentStep === 1 && (
                <div className="space-y-6 md:space-y-8">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
                    ...
                    />
                    <div className="bg-slate-50 p-4 md:p-6 border border-slate-200 rounded-sm">
                      <h4 className="text-xs md:text-sm font-bold text-corporate-900 mb-2 font-serif">Tip</h4>
